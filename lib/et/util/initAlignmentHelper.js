...
(function(et, $){
	...
	function reposition(dom, func){
		...
		if ($.browser.msie && $.browser.version < 7){
			...
			correction = parseInt(el.parent().css('paddingTop')) + parseInt(el.parent().css('paddingBottom'));
		...
		if (el.parent().height() - correction <= el.outerHeight(true) ){
			var css = {};
			css[minHeightProp] = el.outerHeight(true) + correction;
			// expand parent high
			el.parent().css(css);
		}
	}
