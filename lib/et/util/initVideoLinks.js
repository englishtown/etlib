    ...
    function showVideo(content, noFlash, link){
        ...
        if (swfobject.getFlashPlayerVersion().major > 0 ||
            ...
            if (jQuery.support.html5Video){
                ...
                if ((link == null || videoId == null || videoId == false) && window.videoObject && videoObject.para1){
                    videoId = videoObject.para1;
                }
                
                videoIframe = $('<iframe id="' + idbakup + '" class="youtube-player et_video_placeHolder"' +
                    ' type="text/html" src="http://www.youtube.com/embed/' + videoId + '?' + pAutoplay + '&rel=0' + '" frameborder="0"></iframe>');
            ...
            var lightbox = new et.ui.lightbox({
                ...
                onHide: function () {
                    ...
                    // remove iframe
                    // remove 'src' first because the voice is still there in IE9
                    // although you have already removed the iframe element. 
                    if (videoIframe && videoIframe.remove){
                        videoIframe.removeAttr('src').replaceWith(videoContainer).remove();
                    }
            ...
            // return the lightbox for later use.
            ret = lightbox;
        }
        else{
