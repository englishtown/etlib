...
(function (et, $) {
    ...
    ctor.prototype = {
        ...
        */
        show: function () {
            var top, $window, positionTop, _domHeight, windowHeight, exceedHeight;
            ...
            top = (et.dom.getEnv().clientHeight - $(this._dom).outerHeight(true)) / 2;
            top = top > 0 ? top : 0;

            /* If isFixed is true, set 'position' to 'absolute' */
            if(this._options.isFixed){
                $window = $(window);
                positionTop = $window.scrollTop();
                windowHeight = $window.height();
                _domHeight = $(this._dom).height();
                exceedHeight = _domHeight - windowHeight;

                /* 
                    If height of lightbox exceed height of window,
                    make lightbox align bottom with window.
                */
                if(exceedHeight > 0){
                    positionTop -= exceedHeight;
                }

                this._overlay.container.css({
                    'position': 'absolute',
                    'overflow': 'hidden', /* Hide the scrollbar of window */
                    'height': 'auto', /* Original height is 100% */
                    'top': positionTop
                });
            }
