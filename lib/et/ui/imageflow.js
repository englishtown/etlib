...
Author:     Norman Xu (modified based on Finn Rudolph's ImageFlow)
...
function ImageFlow() {
    ...
    this.defaults =
	{
	    aspectRatio: 1.9,          /* Aspect ratio of the ImageFlow container (width divided by height) */
		...
		onChange: function() { }, /* OnChange behaviour */
	    opacity: true,          /* Toggle image opacity */
	    opacityArray: [10,9, 5, 3, 1],   /* Image opacity (range: 0 to 10) first value is for the focussed image */
	    ...
	    scrollbarP: 0.8375,            /* Width of the scrollbar in percent */
	    slider: true,           /* Toggle slider */
	    sliderCursor: 'e-resize',     /* Slider cursor type - default is 'default' */
	    sliderWidth: 37,             /* Width of the slider in px */
	    startID: 100,              /* Glide to this image ID on startup */
    ...
    this.init = function(options) {
        /* Evaluate options */
        var optionsArray = ['aspectRatio', 'buttons', 'captions', 'imageCursor', 'imagesM', 'ImageFlowID', 'imageFocusM', 'imageFocusMax', 'imagesHeight', 'onClick', 'onChange', 'opacity', 'opacityArray', 'percentLandscape', 'percentOther', 'preloadImages', 'reflections', 'reflectionGET', 'reflectionP', 'reflectionPNG', 'imageScaling', 'scrollbarP', 'slider', 'sliderCursor', 'sliderWidth', 'startID', 'startAnimation', 'xStep'];
    ...
    this.refresh = function() {
        ...
        if (!this.sliderWidthCorrected) {
			this.sliderWidth = thisObject.sliderWidth * 0.5;
			this.sliderWidthCorrected = true;
		}
    ...
    this.glideTo = function(imageID) {
		...
		/* fire onChange event */
		thisObject.onChange.call(thisObject, imageID);
    ...
    this.Helper =
	{
	    ...
	    setOpacity: function (object, value) {
	        if (thisObject.opacity === true) {
                ...
                if (object.getAttribute('data-image-flow-alpha') == "1" && object.filters && object.filters.length > 0){
                        object.filters.item("DXImageTransform.Microsoft.Alpha").enabled = 1;
                        object.filters.item("DXImageTransform.Microsoft.Alpha").opacity = value * 10;
                }
                else if(object.runtimeStyle && object.runtimeStyle.filter){
                    object.runtimeStyle.filter = object.runtimeStyle.filter + ' progid:DXImageTransform.Microsoft.Alpha(opacity=' + value * 10 + ')';
                    object.setAttribute('data-image-flow-alpha', "1")
                }
	        }
	    },
